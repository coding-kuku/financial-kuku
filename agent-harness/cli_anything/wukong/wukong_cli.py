"""cli-anything-finclaw — CLI harness for FinClaw (财务虾).

Usage:
    cli-anything-finclaw                          # enter REPL
    cli-anything-finclaw --url http://HOST:PORT   # custom server URL
...
_VERSION = "1.0.0"
_skin = ReplSkin("finclaw", version=_VERSION)
...
def _get_client(ctx: click.Context) -> WukongClient:
    ...
    URL priority: --url flag > FINCLAW_URL env var > session file > default.
    """
...
@click.option("--url", default=None, help="FinClaw server URL (default: http://localhost:44316)")
...
def cli(ctx: click.Context, url: Optional[str], use_json: bool):
    """cli-anything-finclaw — command-line interface for FinClaw (财务虾).

    Run without a subcommand to enter the interactive REPL.

    The FinClaw server must be running at the configured URL.
